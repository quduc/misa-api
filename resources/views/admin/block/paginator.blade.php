            ...
            <div class="col-9 text-right">
                <span class="position-relative">
                    <span class="pr-3">
                        ...
                        $limit = request()->get('limit', \App\Http\Controllers\Admin\CRUDAdminController::LIMIT);
                        $limitOptions = [20 => 20, 40 => 40, 60 => 60, 80 => 80, 100 => 100];
                        if (!in_array($limit, $limitOptions)) {
                            $limitOptions[$limit] = $limit;
                        }
                        ?>
                    表示
                       {{ Form::select('limit', $limitOptions, $limit, ['class' => 'admin-crud-paginator-select','data-url'=> url()->current() . '?' . http_build_query($queries)]) }}
                        件
                        </span>
                    {{-- Previous Page Link --}}
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span
                                            class="text-primary font-weight-bold relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-500  border border-gray-300 cursor-default leading-5">{{ $page }}</span>
                                    </span>
                                @else
                                    <a href="{{ $url }}"
